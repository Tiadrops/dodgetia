(function(){
  ...
  // local utilities (needed because main utilities live in another closure)
  function clamp(v,a,b){ return Math.max(a, Math.min(b, v)); }
  function circleIntersectsRotRect(cx,cy,r,rx,ry,len,wid,angle){ const ca=Math.cos(angle), sa=Math.sin(angle); const tx=cx-rx, ty=cy-ry; const lx=tx*ca+ty*sa, ly=-tx*sa+ty*ca; const nx=Math.max(0, Math.min(len, lx)); const ny=Math.max(-wid*0.5, Math.min(wid*0.5, ly)); const dx=lx-nx, dy=ly-ny; return (dx*dx+dy*dy)<=r*r; }
  function circleInFrontSemicircle(px,py,pr,cx,cy,r,ang){ const dx=px-cx, dy=py-cy; const ca=Math.cos(ang), sa=Math.sin(ang); const lx=dx*ca+dy*sa, ly=-dx*sa+dy*ca; if((lx+pr)<0) return false; return (lx*lx+ly*ly) <= (r+pr)*(r+pr); }
window.Enemies.Hisui = function(cfg){
    const M = cfg.METER;
