...
(function(){
  ...
  // Debi & Marlene enemy factory
  // cfg: { METER, player, bounds:{w,h}, onDanger: ()=>void, sprites?: {debi?: HTMLImageElement, marlene?: HTMLImageElement} }
  Enemies.DebiMarlene = function(cfg){
    const M = cfg.METER;
    const player = cfg.player;
    const W = cfg.bounds.w, H = cfg.bounds.h;
    ...
    // initialize order
    // Base order depends on start form, but R can occur at any position.
    ...
    function fireMarProj(){
      ...
      // Marlene Q projectile is Caution (yellow) hit
    ...
    function triggerDebiDashFrom(px,py, opts){
      const ang = (opts && typeof opts.ang === 'number') ? opts.ang : Math.atan2(player.y - py, player.x - px);
      const ux=Math.cos(ang), uy=Math.sin(ang);
      e.dashes.push({ ox:px, oy:py, ux, uy, elapsed:0, follow: !!(opts && opts.follow), marker: opts && opts.marker });
    }
    function dashUpdate(dt){
      if (!e.dashes || e.dashes.length === 0) return;
      const next = [];
      for (const d of e.dashes) {
        d.elapsed += dt; const t = Math.min(1, d.elapsed / MAR_DASH_TIME);
        const dist = t * MAR_DASH_DIST; const cx = d.ox + d.ux * dist; const cy = d.oy + d.uy * dist;
        const ang = Math.atan2(d.uy, d.ux);
        if (circleIntersectsRotRect(player.x, player.y, player.radius, cx, cy, MAR_SWEEP_LEN, MAR_SWEEP_WID, ang)) cfg.onDanger && cfg.onDanger();
        if (d.follow) { e.x = cx; e.y = cy; e.facing = (d.ux >= 0) ? 1 : -1; }
        if (d.marker === 'Debi' && e.placedDebi) { e.placedDebi.x = cx; e.placedDebi.y = cy; }
        if (t >= 1) {
          if (d.follow) { e.x = cx; e.y = cy; }
    ...
    function updateProjs(dt){
      ...
      for (const p of e.projs){
        p.x += p.vx*dt; p.y += p.vy*dt;
        const dx = p.x - p.sx, dy = p.y - p.sy; if (dx*dx+dy*dy >= p.range*p.range) continue; // reached
        // hit
        const hx = player.x - p.x, hy = player.y - p.y; const rr = (player.radius + p.r);
    ...
    function applyDQ(){
      const ang = e.dq_ang;
      if (circleIntersectsRotRect(player.x, player.y, player.radius, e.x, e.y, Q_DEB_RECT_L, Q_DEB_RECT_W, ang)) {
        cfg.onCaution && cfg.onCaution();
      }
      if (e.placedMarlene) {
        // fire Debi projectile from placed Marlene position
    ...
    function applyDE(){
      // fire Debi projectile from current position
      const ang = Math.atan2(player.y - e.y, player.x - e.x);
      // Debi E projectile should be Caution
    ...
    function applyME(){
      ...
      // create dash anchored at current pos
      const ang = Math.atan2(player.y - e.y, player.x - e.x);
    ...
    function applyR(){
      ...
      e.x = ox + Math.cos(ang)*R_RECT_L; e.y = oy + Math.sin(ang)*R_RECT_L;
      e.facing = (Math.cos(ang) >= 0) ? 1 : -1;
    }
    ...
    function update(dt){
      ...
      switch(e.state){
        case 'spawn_idle': if (e.t>=1.0){ e.state='move'; e.t=0; } break;
        case 'move': {
          // after E, spend 1s moving only. If a self-dash is active, let it drive position.
          if (e.postETime > 0) {
    ...
    function draw(ctx){
      ...
      // dash telegraph rectangle while active
      if (e.dashes && e.dashes.length){
        for (const d of e.dashes){
          ...
          ctx.save(); ctx.translate(cx,cy); ctx.rotate(ang); ctx.globalAlpha=0.35; ctx.fillStyle='rgba(239,68,68,0.25)'; ctx.strokeStyle='rgba(239,68,68,0.9)'; ctx.lineWidth=2; ctx.beginPath(); ctx.rect(0,-MAR_SWEEP_WID/2,MAR_SWEEP_LEN,MAR_SWEEP_WID); ctx.fill(); ctx.stroke(); ctx.restore();
        }
      }
      ...
      if (e.state==='DQ_cast' && (Q_DEB_CAST - e.t) <= 0.05){
        ...
        ctx.beginPath(); ctx.rect(0, -Q_DEB_RECT_W/2, Q_DEB_RECT_L, Q_DEB_RECT_W); ctx.fill(); ctx.stroke();
        ctx.restore();
      }
